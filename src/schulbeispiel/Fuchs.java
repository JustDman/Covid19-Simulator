...
/**
 * Ein simples Modell eines Fuchses. Füchse altern, bewegen sich, fressen Hasen
 * und sterben.
 * 
 * @author David J. Barnes und Michael Kölling
 * @version 2016.03.18
 */
public class Fuchs extends Organism {
    ...
    /**
     * Erzeuge einen Fuchs. Ein Fuchs wird entweder neugeboren (Alter 0 Jahre und
     * nicht hungrig) oder kann mit einem zufälligen Alter und zufälligem
     * Hungergefühl erzeugt werden.
     * 
     ...
     */
    public Fuchs(boolean zufaelligesAlter, Field feld, Position position) {
        super(feld, position);
        if (zufaelligesAlter) {
    ...
    /**
     * Das ist was ein Fuchs die meiste Zeit tut: er jagt Hasen. Dabei kann er
     * Nachwuchs gebären, vor Hunger sterben oder an Altersschwäche.
     * 
     * @param neueFuechse eine Liste zum Zurückliefern neugeborener Füchse
     */
    public void act(List<Organism> neueFuechse) {
        alterErhoehen();
        hungerVergroessern();
        if (isAlive()) {
            ...
            if (neuePosition == null) {
                ...
                neuePosition = getField().freeNeighborPosition(getPosition());
            }
    ...
    /**
     * Erhöhe das Alter dieses Fuchses. Dies kann zu seinem Tod führen.
     */
    ...
    /**
     * Vergrößere den Hunger dieses Fuchses. Dies kann zu seinem Tode führen.
     */
    ...
    /**
     * Suche nach Nahrung (Hasen) in den Nachbarpositionen. Es wird nur der erste
     * lebendige Hase gefressen.
     * 
     * @return die Position mit Nahrung, oder null, wenn keine vorhanden
     */
    private Position findeNahrung() {
        Field feld = getField();
        List<Position> nachbarPositionen = feld.neighborPositions(getPosition());
        Iterator<Position> iter = nachbarPositionen.iterator();
        while (iter.hasNext()) {
            Position pos = iter.next();
            Object tier = feld.getObjectAt(pos);
    ...
    /**
     * Prüfe, ob dieser Fuchs in diesem Schritt gebären kann. Neugeborene kommen in
     * freie Nachbarpositionen.
     * 
     * @param neueFuechse eine Liste zum Zurückliefern neugeborener Füchse
     */
    private void gebaereNachwuchs(List<Organism> neueFuechse) {
    ...
    /**
     * Erzeuge eine Zahl für die Wurfgroesse, wenn der Fuchs gebaeren kann.
     * 
